            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    ...
                    {pendingItems.map(item => (
                        <div key={item.id} onClick={() => navigate(`/itemReport/${item.id}`)} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 bg-white dark:bg-gray-900 p-5 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row gap-5 transition-colors">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex gap-3">
                                    <button onClick={(e) => { e.stopPropagation(); handleVerifyItem(item.id, true); }} className="flex-1 bg-brand-green text-white py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:opacity-90">
                                        <Check size={16} /> Approve
                                    </button>
                                    <button onClick={(e) => { e.stopPropagation(); handleVerifyItem(item.id, false); }} className="flex-1 bg-red-50 dark:bg-red-900/20 text-brand-red py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-100 dark:hover:bg-red-900/30">
                                        <X size={16} /> Reject
                                    </button>
                                </div>
                            </div>
                ...
                <div className="space-y-4">
                    ...
                    {pendingClaims.map(({ claim, item }) => (
                        <div key={claim.id} onClick={() => navigate(`/itemReport/${item.id}`)} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
                            ...
                            <div className="flex gap-3">
                                <Button fullWidth onClick={(e) => { e?.stopPropagation?.(); handleProcessClaim(claim.id, true); }}>Approve Claim</Button>
                                <Button fullWidth variant="secondary" onClick={(e) => { e?.stopPropagation?.(); handleProcessClaim(claim.id, false); }}>Reject</Button>
                            </div>
                        </div>
                    ))}
                </div>
